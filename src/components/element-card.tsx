...
import {
  extractElementByLocator,
  extractElementDataModelFromQuote,
} from "@socotra/ec-react-utils";
import { JsonEditor } from "json-edit-react";

import { dataModel, quote } from "@/config/constraints";
...
export const ElementCard = (props: ElementCardProps) => {
  ...
  const element = extractElementByLocator(quote, "01JAEMS2D8BKQN8MGEC5MGJDK3");

  if (!element) {
    return null;
  }

  const elementModel = extractElementDataModelFromQuote({
    dataModel,
    quote,
    elementLocator: "01JAEMS2D8BKQN8MGEC5MGJDK3",
  });
          ...
          <ResizablePanel className="px-2">
            <div className="flex h-[64rem] flex-col gap-4 overflow-y-auto px-8">
              <div className="flex flex-col gap-2 py-2">
                <span className="text-md font-bold">Element Form</span>
                <ElementForm
                  handleSubmit={(data) => {
